...
| `skills/gpu-shader-toolkit` | 通用 GPU shader 二进制检查、反汇编、交叉编译、captured shader 移植与证据记录工具链。 | 统一发现和调用 DXC、SPIRV-Cross、glslang、SPIRV-Tools、dxil-spirv；支持 DXIL/DXBC dump、SPIR-V 编译/反汇编/reflection、HLSL 编译；记录原始/反汇编/翻译/引擎适配四层产物、hash、常量语义、sampler/packed-format 和 translation deviation。 | 支持，仓库内已带 DXC 最小运行集和四个 SPIR-V CLI；`spirv-val/spirv-opt` 仍从 PATH、Vulkan SDK 或显式 toolchain 补齐。 | 流程可用；可使用 Homebrew/Vulkan SDK/本机工具，当前未内置 macOS 二进制。 | 分析 PIX/RenderDoc/GPA 导出的 shader；检查 DXIL/DXBC/SPIR-V；对照 resource binding 和 root signature；把 captured shader 可审计地移植到另一 API/引擎；复现实验 shader 编译参数。 |
| `skills/pix-cpp-export-toolkit` | 通用 PIX on Windows `.wpix` 截帧准备、C++ 导出索引与逆向证据链工作流。 | 发现并调用 `pixtool.exe`；把 `.wpix` 转成 event list 与 C++ replay；封装 RTV/depth `save-resource`；验证导出结构；重建 `resourceReader` 读流；索引 PSO、shader bytecode、CommandLists 命令事件和候选 draw/dispatch；解压 `resources.bin` 中的 PSO shader blob、resource 初始化块或任意函数读块；输出 Markdown/JSON/manifest 证据。 | 支持，`.wpix` 准备与 C++ 导出依赖本机 PIX on Windows；索引脚本是 Python，`resources.bin` 解压使用 Windows Cabinet XPRESS，shader 反汇编可选用 `dxc.exe`。 | 可分析已经复制过来的 C++ 导出文件；PIX 导出和 XPRESS 解压仍依赖 Windows/PIX，任意 GUI Resource History 导出或领域语义解码脚本需另行补齐。 | 给 AI 一个 `.wpix` 路径后自动准备证据；将 PIX C++ 导出变成可复查报告；定位渲染/计算 pass 候选；追踪 PSO、root 参数、descriptor、resource id；提取 shader/resource 原始证据；判断哪些 GUI 操作不能由 `pixtool` 直接替代。 |
| `skills/renderdoc-mcp-skill` | RenderDoc MCP 薄网关与 `.rdc` 精确证据导出工作流。 | 发现、安装和配置 `renderdoc-mcp`；分层诊断注册、transport、runtime 与具体 capture 兼容性；按 EID 导出 pipeline、descriptor、sampler、raw texture/buffer、shader 和 hash manifest；生成选定 pass 的资源依赖报告；提供跨引擎重建验收约束。 | 支持，MCP/CLI 依赖已安装或已验证的 release；精确导出依赖本机 RenderDoc `qrenderdoc` Python API。 | 工作流可用，但需目标平台存在兼容的 RenderDoc replay 与 `qrenderdoc`；不随 skill 打包完整 RenderDoc。 | RenderDoc 服务诊断；新旧 capture 兼容性排查；RDC shader/resource 证据落盘；captured effect 重建前的 pass graph、格式和坐标契约。 |
...
| `skills/unity-global-installer` | Unity Editor 全局模块安装与修复工作流。 | 发现 Windows/macOS/Linux 的 Hub Editor；读取 `modules.json`；列出任意模块 ID；诊断 Unity CDN/镜像重定向；下载并按平台计划/执行 `.exe`、`.pkg`、`.zip` 模块安装；比较 donor Editor 模块 URL；安全复用兼容 payload；标记 selected；用 batchmode 验证目标平台真实加载。 | 支持，依赖本机 Unity Hub Editor、Python 3，写入 `Program Files` 时需要管理员权限。 | 支持主流程，依赖本机 Unity Hub Editor、Python 3，`.pkg` 安装需 `sudo`；未知安装器类型只生成计划。 | Unity Hub 安装 Android、iOS、WebGL、Dedicated Server、桌面 Build Support 等模块失败；模块下载 404/镜像错误；跨版本模块状态修复与验收。 |
